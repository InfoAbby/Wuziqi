...
public class Ai implements I_Ai{
    ...
    @Override
    public Pos action(int[][] chess, int depth) {
        List<Pos> points;//particalpoints container
        List<Pos> bestpoints=new ArrayList<>();//bestpoints container
        ...
        {
            Pos bp=new Pos();
            bp.setX(points.get(i).getX());
            bp.setY(points.get(i).getY());
            Log.d(TAG, "particalPos:"+bp.getX()+" "+bp.getY());
            chess[bp.getX()][bp.getY()] = 1;//try it
            ...
            chess[bp.getX()][bp.getY()] = 0;//renew it
        }
        //choose one randomly
        long t=System.currentTimeMillis();
        Random random=new Random();
        int size = random.nextInt(1000)%bestpoints.size();
        chess[bestpoints.get(size).getX()][bestpoints.get(size).getY()] = 1;
        Log.d(TAG, "bestChoice "+bestpoints.get(size).getX()+" "+bestpoints.get(size).getY());
        Log.d(TAG, "bestChoice Value "+best);
        Log.d(TAG, "total"+ ConfigData.TOTAL);
        Log.d(TAG, "cutting points"+ConfigData.ABCUTS);
        Pos bs=new Pos();
        bs.setX(bestpoints.get(size).getX());
        bs.setY(bestpoints.get(size).getY());
        return bs;
    }
    ...
    public int max(final int[][] chess, final int deep, final int alpha, final int beta) {
        ...
        if(deep<=0 || isWin(chess)!=0)
            return tbest;
        int best =ConfigData.MIN;
        final List<Pos> points = generate(chess,deep);
        for(int k = 0; k < points.size(); k++)
        {
            int x = points.get(k).getX();
            int y = points.get(k).getY();
            chess[x][y] = 1;//try it
    ...
    @Override
    public List<Pos> generate(int[][] chess, int deep) {
        List<Pos> points=new ArrayList<>();
        List<Pos> tpoints=new ArrayList<>();
        List<Pos> five=new ArrayList<>(),
